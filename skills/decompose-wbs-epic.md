...
description: "Decompose a WBS epic into smaller, independent, vertical user stories (INVEST), each written as a full 'As a role, I want, so that' statement and grouped under its parent WBS story. Reads the WBS spreadsheet and Product Context, names the splitting pattern used, and surfaces split decisions and WBS-vs-PC gaps for BA review."
---
...
1. **WBS** — `/mnt/project/WBS_Scope_only_1.xlsx`, sheet **`WBS & Development Efforts`**.  
   ...
   - Extract with code, e.g.:  
     ...
     ```  
2. **Product Context** — `/mnt/project/Product_Context_actual.md`. The authoritative source for content: read the **role list**, **user states**, and the **functional behaviour** of the epic's domain. Where the Product Context and the WBS raw AC conflict, the Product Context wins.  
